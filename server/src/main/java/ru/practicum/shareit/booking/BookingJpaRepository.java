...
public interface BookingJpaRepository extends JpaRepository<Booking, Long> {
    ...
    List<Booking> findAllByBookerIdAndStatus(Long userIdHeader, BookingStatus bookingStatus, Pageable pageable);

    @Query("select booking from Booking booking where booking.booker.userId = ?1 order by booking.bookingStart desc")
    List<Booking> findAllByBookerIdAndStateAll(Long userIdHeader, Pageable pageable);

    @Query("select booking from Booking booking where booking.booker.userId = ?1 and booking.bookingEnd < ?2 " +
            "order by booking.bookingStart desc")
    List<Booking> findAllByBookerIdAndStatePast(Long userIdHeader, LocalDateTime nowTime, Pageable pageable);
    ...
    List<Booking> findAllByBookerIdAndStateCurrent(Long userIdHeader, LocalDateTime nowTime, Pageable pageable);

    @Query("select booking from Booking booking where booking.booker.userId = ?1 and booking.bookingStart > ?2 " +
            "order by booking.bookingStart desc")
    List<Booking> findAllByBookerIdAndStateFuture(Long userIdHeader, LocalDateTime nowTime, Pageable pageable);
    ...
    List<Booking> findAllByItemsOwnerIdAndStatus(Long userIdHeader, BookingStatus bookingStatus, Pageable pageable);

    @Query("select booking from Booking booking left join Item as item on booking.bookingItem.itemId = item.itemId " +
            "where item.owner.userId = ?1 order by booking.bookingStart desc")
    List<Booking> findAllByItemsOwnerIdAndStateAll(Long userIdHeader, Pageable pageable);

    @Query("select booking from Booking booking left join Item as item on booking.bookingItem.itemId = item.itemId " +
            "where item.owner.userId = ?1 and booking.bookingEnd < ?2 order by booking.bookingStart desc")
    List<Booking> findAllByItemsOwnerIdAndStatePast(Long userIdHeader, LocalDateTime nowTime, Pageable pageable);

    @Query("select booking from Booking booking left join Item as item on booking.bookingItem.itemId = item.itemId " +
            "where item.owner.userId = ?1 and booking.bookingStart < ?2 and booking.bookingEnd > ?2 order by booking.bookingStart desc")
    List<Booking> findAllByItemsOwnerIdAndStateCurrent(Long userIdHeader, LocalDateTime nowTime, Pageable pageable);

    @Query("select booking from Booking booking left join Item as item on booking.bookingItem.itemId = item.itemId " +
            "where item.owner.userId = ?1 and booking.bookingStart > ?2 order by booking.bookingStart desc")
    List<Booking> findAllByItemsOwnerIdAndStateFuture(Long userIdHeader, LocalDateTime nowTime, Pageable pageable);
